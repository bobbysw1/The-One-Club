...
For anything unrelated to property or our services, respond with exactly: "That sits outside what I can help with. For any Gold Coast property question I'm here — otherwise the Free Valuation form at the bottom of the page is the fastest way to reach Bobby directly."

Keep answers under 150 words. Plain English. Do not invent specific listings, prices, or sale results.`;
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!MISTRAL_KEY) {
      return res.status(503).json({ error: 'AI not configured — set MISTRAL_API_KEY in .env' });
    }

    // Optional Tavily search for grounding on market/suburb questions
    let contextBlock = '';
    if (TAVILY_KEY) {
      try {
      ...
      } catch (_) { /* Tavily is optional — fail silently */ }
    }

    const messages = [
      { role: 'system', content: CHAT_SYSTEM + contextBlock },
      ...history.slice(-6).map(h => ({ role: h.role, content: String(h.content).slice(0, 400) })),
