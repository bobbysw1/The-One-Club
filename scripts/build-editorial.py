...
for slug, (lat, lng, name) in SUBURB_COORDS.items():
    ...
    html = p.read_text()
    if 'map-placeholder' in html or 'Open in Google Maps' in html:
        continue  # already added (old iframe version or current link version)
    # Premium external link instead of an embedded iframe, no API key
    # shipped in public HTML, no Maps Embed API billing per pageview.
    # Matches the pattern used in generate_suburb_pages.py.
    import urllib.parse
    q = urllib.parse.quote(f'{name}, QLD, Australia')
    maps_search_url = f'https://www.google.com/maps/search/?api=1&query={q}'
    map_html = f'''
<section style="padding:0 24px 40px;max-width:1100px;margin:0 auto">
  <div data-reveal style="background:rgba(var(--fg-rgb),.025);border:1px solid rgba(var(--fg-rgb),.08);border-radius:18px;padding:32px;display:flex;flex-direction:column;align-items:center;text-align:center">
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:16px"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
    <h3 class="serif" style="font-size:clamp(22px,2.8vw,32px);margin-bottom:12px;font-weight:400">Where is <em>{name}?</em></h3>
    <p class="body" style="font-size:16px;max-width:500px;margin-bottom:24px">Explore the neighbourhood, nearby amenities, and street view on Google Maps.</p>
    <a class="btn btn-outline" href="{maps_search_url}" target="_blank" rel="noopener noreferrer" style="border-color:rgba(var(--fg-rgb),.2)">
      Open in Google Maps
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-left:6px;vertical-align:-2px"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>
    </a>
  </div>
</section>
'''
